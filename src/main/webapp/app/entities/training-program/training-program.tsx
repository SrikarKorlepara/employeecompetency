  ...
  return (
    <div>
      <h2 id="training-program-heading" data-cy="TrainingProgramHeading">
        <Translate contentKey="employeeinfoApp.trainingProgram.home.title">Training Programs</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="employeeinfoApp.trainingProgram.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/training-program/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            ...
            &nbsp;
            <Translate contentKey="employeeinfoApp.trainingProgram.home.createLabel">Create new Training Program</Translate>
          </Link>
        </div>
      </h2>
      <div className="table-responsive">
        {trainingProgramList && trainingProgramList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="employeeinfoApp.trainingProgram.id">ID</Translate>{' '}
                ...
                <th className="hand" onClick={sort('trainingId')}>
                  ...
                  <FontAwesomeIcon icon={getSortIconByFieldName('trainingId')} />
                </th>
                <th className="hand" onClick={sort('trainingName')}>
                  <Translate contentKey="employeeinfoApp.trainingProgram.trainingName">Training Name</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('trainingName')} />
                </th>
            ...
            <tbody>
              {trainingProgramList.map((trainingProgram, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`/training-program/${trainingProgram.id}`} color="link" size="sm">
                      {trainingProgram.id}
                    </Button>
                  </td>
                  <td>{trainingProgram.trainingId}</td>
                  <td>{trainingProgram.trainingName}</td>
                  <td>{trainingProgram.description}</td>
                  <td>
                    {trainingProgram.startDate ? (
                      <TextFormat type="date" value={trainingProgram.startDate} format={APP_LOCAL_DATE_FORMAT} />
                    ) : null}
                  </td>
                  <td>
                    {trainingProgram.endDate ? (
                      <TextFormat type="date" value={trainingProgram.endDate} format={APP_LOCAL_DATE_FORMAT} />
                    ) : null}
                  </td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/training-program/${trainingProgram.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="employeeinfoApp.trainingProgram.home.notFound">No Training Programs found</Translate>
            </div>
          )
        )}
      </div>
